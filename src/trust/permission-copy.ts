...
export const KNOWN_PERMISSION_COPY: Record<string, PermissionCopy> = {
  ...
  media_narrate_clip: { label: 'Narrate one of your videos', detail: 'Watch a video clip already on this machine, write a commentary for it and save a narrated copy beside it. The clip is analysed with your Google AI Studio key; nothing else is uploaded.' },
   ...
   media_generate_rig_parts: { label: 'Generate rig parts', detail: 'Make separate arm parts for a cutout animation rig with Gemini image generation, measure where their joints are and line them up with the rig. Sends the character\'s reference art to Google and costs about US$0.067 per image on your Gemini key. Saves to a review folder in HomeBot; Ancient Pathways is not changed.' },
  ...
  media_trim_clip: { label: 'Trim a video clip', detail: 'Extract a time range from a video file inside your user folder. Writes the trimmed copy beside the source without re-encoding.' },
  media_splice_video: { label: 'Splice video clips', detail: 'Join multiple video files in sequence into a single file inside your user folder using stream copy.' },
  media_create_storyboard: { label: 'Create a visual storyboard', detail: 'Plan a structured multi-shot scene with camera framing, pacing, prompts, and dialogue lines.' },
  ...
  media_set_storyboard_image: { label: 'Set storyboard shot image', detail: 'Attach an image from disk to a shot in the visual storyboard.' },
