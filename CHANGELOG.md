...
#### Added
- **Simple / Advanced view in Settings** (`SettingsPanel.tsx`, `settings/*`): the panel had grown to nine sections in one 2,725-line function, with the settings most people need interleaved with MCP server ports, RAG chunking and the telemetry audit log. Simple is the default and shows which model answers, the privacy switch, voice and appearance; Advanced shows everything, unchanged. The choice is remembered. `SettingsPanel.tsx` is now 156 lines, with the state in `settings/useSettingsState` and the controls in five tab components reading it through a context.
- **One API key per cloud provider** (`shared/cloud-llm.ts`, `config-manager.ts`, `settings/*`): `providerApiKeys` stores a key for each of the thirteen providers the picker offers, encrypted per value at rest and merged on save so an omitted provider means "unchanged". The panel lists which services have a key, with a remove control on each. The four legacy fields (`anthropicApiKey`, `openaiApiKey`, `geminiApiKey`, `moonshotApiKey`) are still written and read as a fallback.
- **In-chat recovery when the local model fails** (`router/error-recovery.ts`, `MessageBubble.tsx`): if a local chat fails or times out and a cloud provider is already configured with a usable key, the error bubble offers to switch. The main process decides whether to offer it, via the same `resolveCloudLLM` the router uses. Never offered when no provider is configured, when the key or model is missing, when cloud is already in use, or while uncensored mode is on.
- **`tests/powershell/Test-SafetyValidation.ps1`**: 25 assertions covering the whitelist, the fail-closed paths, all three `-File` argument-binding forms and portability against a foreign `USERPROFILE`. Deliberately Pester-free so it runs on a fresh clone without `Install-Module`.

#### Fixed
- **The PowerShell safety whitelist had never validated a path** (`scripts/tools/powershell/SafetyValidation.ps1`): all three n8n callers threw on argument binding before any validation ran — an undeclared `-RulesPath`, a `[hashtable]` parameter receiving a JSON string, and a `[bool]` parameter receiving n8n's literal `$false`. The only surviving call form passed no parameters, so it had nothing to check and returned `is_valid: true`. Parameters are now `[object]` and normalised by hand, because every argument arrives as a string through `powershell -File`. Unparseable input fails closed.
- **Hardcoded developer paths** (`config/safety-rules.json`, `SafetyValidation.ps1`, `user-paths.ts`): `%USERPROFILE%` expands at run time. One resolution order everywhere — `USERPROFILE → HOME → os.homedir()`; the old chain preferred `HOME` and fell back to `''`, which made `isPathAllowed` deny every path.
- **`"C:\$Recycle.Bin"` was double-quoted**, so `$Recycle` interpolated to empty and the rule blocked `C:\.Bin`.
- **`Join-Path` resolves the drive**, throwing "Cannot find drive" for a profile on an unmounted volume — the exact portability case the gate exists to survive.
- **The privacy switch was unreachable in Settings** (`settings/PrivacySwitch.tsx`): the only `useCustomLLM` control sat behind `{isConnected && …}`, where `isConnected` means a model list was fetched in the current session — and that list is cleared whenever the panel opens. It is now its own block at the top, in both views, always rendered.
- **Switching cloud provider carried the previous provider's API key across** (`settings/CloudProviderSection.tsx`): the field was seeded from the provider being switched *away from*, so OpenAI → Groq left an `sk-…` OpenAI secret in a config aimed at `api.groq.com`. Switching to a provider with no saved key now leaves the field empty.
- **The Claude subscription option vanished from the default Settings view** — a regression from the Simple/Advanced split, since the cloud provider picker lived in the Advanced tab. The picker moved to `CloudProviderSection`, rendered by `ModelsSettingsTab`, which both views show.
- **Media Studio fullscreen did nothing** (`chatgpt-theme.css`): `.ms-video`'s `max-height: 15rem` is an author rule and beat the UA stylesheet's fullscreen sizing, leaving the video a small letterbox on a black screen.
- **`scripts/patch-workflow-paths.js`** anchors on the repo-relative directory following the root rather than one hardcoded literal, so it still works after the repo has been patched once and moved again; it scans `n8n-workflows` recursively (it was missing the root and `starters/`) and gained a `--check` mode.

#### CI
- **Every e2e job now has a `timeout-minutes`** (`widget-e2e-ci.yaml`, `playwright-e2e.yml`). `e2e-all` is a required check and the job had no timeout, so the default was six hours: two Ubuntu shards hung in `playwright install --with-deps` for over ninety minutes while `gh pr checks` reported "pending", indistinguishable from a slow shard. Step-level timeouts name the step that hangs instead of blaming the suite.
- **Playwright retries reduced from 3 outer attempts to 2.** Wrapped around Playwright's own `--retries=2`, three attempts meant up to nine runs of every failing test.
