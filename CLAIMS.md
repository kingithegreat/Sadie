...
|---|---|---|---|
| DeepSeek model discovery — show current models for a DeepSeek key | claude/deepseek-model-discovery | In progress | Replace the stale static `deepseek-chat`/`deepseek-reasoner` list with dynamic discovery via DeepSeek's `/models` endpoint when a key is present; cache briefly; fall back to a small current list (`deepseek-v4-flash`/`deepseek-v4-pro`); update the first-run default model. |
