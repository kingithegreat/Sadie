...
|---|---|---|---|
| Narrate clip bug fix — pass video path to Python analyzer | claude/provider-correctness | Ready for integration — **PR #272 open (2026-09-09)** | Fixed `narrate-clip.ts:196` where `buildAnalyzerArgs(analyzerScript, analysisOut)` was called instead of `[analyzerScript, ...buildAnalyzerArgs(videoPath, analysisOut)]`. The video path was missing, causing the Gemini clip analyzer to receive the script path as input. All 9 tests pass (verified locally); tsc and eslint clean. Reachable via `media_narrate_clip` registered by `modules/bundled/studio.ts`. |
