...
|---|---|---|---|
| Chat → agent bridge proof: Gemini dynamic model discovery | claude/gemini-model-discovery | Ready for integration — **PR #273 open (2026-09-09)** | Replace the static `GOOGLE_AI_MODELS` / `GOOGLE_GEMINI_NATIVE_MODELS` reliance with dynamic discovery via Google's `models.list` endpoint when a key is present; filter to `generateContent`-capable models, keep `supportedGenerationMethods` as separate `capabilities` metadata, cache briefly, fall back to the small static list on failure. Never infer account tier/entitlement from the list. |
