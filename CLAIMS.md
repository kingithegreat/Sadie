...
|---|---|---|---|
| Automation event triggers (Track H) | claude/automation-event-triggers | In progress — ox-alpha, working in sadie-ox2 worktree | Aden's tasking. Area exclusive today: `widget/src/main/tools/automation.ts`, `widget/src/main/scheduler.ts`, `widget/src/renderer/components/AutomationCenter.tsx`. Goal: at least one event trigger end to end (type + UI + the thing that actually fires it) beyond manual+schedule; edit parity (#212) already shipped, not redone. The n8n surface question is a product call — raised with Aden before building anything on that half. |
