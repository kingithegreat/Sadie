...
| Feature | Branch | Status | Notes |
|---|---|---|---|
| ~~HB-M2 — narration privacy and real saved storyboard export~~ | claude/studio-narrated-export | Local implementation verified; build claim released — Codex 2026-09-09 | Includes preserved speech/export snapshots from `codex-speech-privacy` and `codex-studio-export`; both original worktrees remain intact. Current-main integration `f096a80` passes 3,943 widget tests (15 existing skipped), 227 unchanged root tests and all 56 actual Electron tests without retries. An actual 8-second 1080p movie has images, timed CPU narration, captions, restart playback and safe failed/successful replacement. Evidence: `tasks/studio-narrated-export.md` and `docs/evidence/studio-narrated-export.json`. The existing Notion speech/export records retain integration ownership until required CI and merged content are verified. Full HB-M2 remains open. |
...
## 2026-09-09 — narration and saved movie export (Codex)

The owner-authorized combined speech/export implementation passes all local
gates on current main, including 56 actual Electron cases and a real narrated
MP4. Its build claim is released; the existing Notion speech/export records
track required CI and merged-content verification. Evidence and limits are in
`tasks/studio-narrated-export.md`. Full M2 and the independently owned lazy
runtime remain open.

Keep speech consent at the shared adapter, including model/tokenizer loads,
voice listing and fallback. Export the same saved shot files/order that Studio
displays, rather than the director's original manifest. Release the video
player's Windows file handle before replacing a validated export; preserve the
previous movie and restore its player after failure.
