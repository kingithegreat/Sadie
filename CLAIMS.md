...
| Visual pass: icons redrawn + chrome polish | claude/visual-pass | Ready for integration | Aden asked for a visual pass; the icon glyphs were the named complaint. All 35 `IconName` entries redrawn to Lucide-grid geometry in components/Icon.tsx (worst offenders: settings gear was a lumpy hand-trace, sparkle read as an asterisk) — union unchanged so ~40 call sites need no edit, no new deps. Plus an appended ICON & CHROME POLISH PASS block at the end of chatgpt-theme.css (same removable-block discipline as the depth pass): .hb-icon optical alignment (block + flex-shrink:0), one shared affordance model across header-btn/mode-btn/attach-button/voice-button/code-copy-btn (hover lift, press sink, :focus-visible ring), disabled-state honesty, on-palette ::selection, reduced-motion respected. Zero layout/size changes. Worktree: Desktop/sadie-visual. |
