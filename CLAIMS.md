...
|---|---|---|---|
| Narrate clip bug fix — pass video path to Python analyzer | claude/provider-correctness | Implemented | Fixed `narrate-clip.ts:196` where `buildAnalyzerArgs(analyzerScript, analysisOut)` was called instead of `[analyzerScript, ...buildAnalyzerArgs(videoPath, analysisOut)]`. The video path was missing, causing the Gemini clip analyzer to receive the script path as input. All 9 tests pass. |
