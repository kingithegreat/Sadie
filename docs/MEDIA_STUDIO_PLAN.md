...
- **Colab:** an operator-assisted, pending-ticket-interruptible GPU worker for expensive asset
  ...
  a pending ticket does not stop a notebook cell already running; its late result
  is ignored and the operator stops the notebook manually if needed.
...
| **4. Colab round trip** — backend merged in PR [#298](https://github.com/kingithegreat/Sadie/pull/298); REL-3.4 queue UI implemented pending PR/merge | Portable unique jobs, asset upload, Drive discovery, worker result import, project-scoped list/retry/pending-cancel, and an explicit manual-worker opt-in. The existing movie run remains the resume/import path; unified recovery across the separate Media Studio pipelines remains open. | Submit from HomeBot, run in Colab, ingest validated assets and continue the same project; survive runtime restart and duplicate/partial results. Cancellation must ignore late output without claiming to stop an active notebook. |
