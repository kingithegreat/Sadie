...
- **Colab:** an operator-assisted, pending-ticket-interruptible GPU worker for expensive asset
  ...
  leaves a resumable job, not a failed or fabricated finished movie. Cancelling
  a pending ticket does not stop a notebook cell already running. Retry uses a
  new attempt identity/output path, so an older worker's late result is isolated
  from the current attempt; the operator stops the notebook manually if needed.
...
| **4. Colab round trip** — backend merged in PR [#298](https://github.com/kingithegreat/Sadie/pull/298); REL-3.4 queue UI implemented in draft PR #403, with local packaged Electron acceptance passed 2026-09-21 and the race-fix follow-up not yet pushed | Portable unique jobs, asset upload, Drive discovery, worker result import, project-scoped list/retry/pending-cancel, explicit manual-worker opt-in, and attempt-scoped retry outputs. The existing movie run remains the resume/import path; unified recovery across the separate Media Studio pipelines remains open. | Submit from HomeBot, run in Colab, ingest validated assets and continue the same project; survive runtime restart and duplicate/partial results. Cancellation must isolate late output from the current attempt without claiming to stop an active notebook. |
