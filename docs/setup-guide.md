...
| **Image** | Ctrl+3 | Type a prompt to generate an image (auto-detects local engines: stable-diffusion.cpp, SD WebUI, ComfyUI; then Pollinations or Stable Horde; OpenAI GPT Image if a key is set) |
