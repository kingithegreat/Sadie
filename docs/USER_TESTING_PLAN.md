...
| MS-8 | **Smooth camera motion:** FFmpeg `zoompan` on a same-size image steps in whole pixels, which visibly jitters on slow push-ins. Standard fix: upscale the source before `zoompan` (or use a sub-pixel crop/scale expression). | A/B on the same shot: frame-to-frame motion is monotonic without judder (measured motion vectors or pixel shift per frame), render-time cost recorded. | quality | merged #376 for Storyboard; #395 shares the supersampled filter with the job pipeline and measures the job path live (pan judder 0.361 → 0.004) |
