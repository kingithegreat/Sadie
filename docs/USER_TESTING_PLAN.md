...
| IDE-9 | **Search across files** UI (Ctrl+Shift+F) with replace-in-files preview, on top of `search.ts` / `grep_code`. | Search finds matches in several files; replace preview then apply changes exactly those lines. | PR #397 |
