...
| IDE-3 | **Agent edits you can accept or reject:** multi-file changes shown as per-file and per-hunk diffs with Accept / Reject before they're written, extending `ChangesPanel`. | Rejecting a hunk leaves the file byte-identical; accepting writes exactly the shown hunk. | PR #378 |
...
| IDE-5 | **Inline edit (Ctrl+K)** on a selection: prompt → diff preview in place → accept/reject. | The selected range is replaced only on accept, and undo restores it. | in progress (claude/ide-5-inline-edit) |
