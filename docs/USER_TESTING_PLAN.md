...
### Current authority and integration checkpoint

The [HomeBot current Drive plan](https://docs.google.com/document/d/1sh2ss0epsUxe6VcbF7GwULs4_TFgSPJh9R_I46xYoH8/edit) is the single live priority queue; this file retains the detailed acceptance requirements. Preserved Notion pages are history, not a competing schedule. The checkpoint below supersedes older status descriptions; a merged implementation does not by itself prove its runtime acceptance.

- **First production priority: MS-RIG-0.** Preserve approved Leila artwork and the existing Ancient Pathways rig owner. Use copied, revisioned assets for the six-second, 1920×1080, 30 fps, captions-off mechanics proof: still torso, bending elbow, attached shoulder/wrist, stable expression registration and a held prop. Do not re-slice Season 1, overwrite `_mouth_anchors`, start a competing renderer or touch the owner's dirty rig tree. Owner visual acceptance precedes the two-character proof and full episode acceptance.
- **Milestone A stays open:** create/edit/save/close/reopen/export and inspect a complete episode, repeat its export, then inject a re-render failure in an isolated project/profile and prove the last good output remains intact and playable after restart. Retain artifact paths/checksums and owner visual/audio review.
- **Milestone B stays open:** all customization, Code mode and release requirements retain their individual acceptance criteria. New packages, model downloads, external processing, paid requests and design decisions remain approval-gated.
- **G-1: 2/3 scheduled passes.** Runs `35438194068` and `35506934625` are consecutive scheduled successes. The latter ran on `c15c69f`, built the widget, passed 338 suites / 4,572 tests (31 skipped), and passed both real-app smoke tests. A third scheduled success is still required; a manual run would not establish a third scheduled day.
- **G-2: 3/3 scheduled passes.** Runs `35318304278`, `35428550024` and `35497242759` supply the recorded three consecutive scheduled successes. This is nightly-media evidence, not installer or full-episode acceptance.
...
- **REL-3.4:** merged as #403 at `e1ac128`. All six required contexts, all nine E2E shards and `e2e-all` passed on tested head `02d86f4`; the landed tree exactly matches it. Independent local and CI widget runs each passed 338 suites / 4,601 tests (31 skipped), alongside 90 focused tests and disposable-profile real Electron acceptance (1/1, no retries), including restart. The commissioned operator-assisted worker is retained; live Colab generation, active-notebook interruption and unified cross-pipeline recovery are not claimed.
...
| RAM | 15.7 GB |
| FFmpeg | managed n9.0 build. Encoders present: `h264_nvenc`, `hevc_nvenc`, `av1_nvenc`, `h264_qsv`, `libx264`. Storyboard and job rendering share probed encoder selection, with `libx264` fallback. Encoder presence alone is not runtime render proof. |
| OS | Windows 11 |
...
- Hardware H.264 selection is implemented; keep the real probe and CPU fallback. Record the actual encoder used for each acceptance render.
...
| G-4 | Required `e2e-all` waits on queued macOS/Windows shards, often for hours, which makes every PR slow to land. | The queue time is measured, and a proposal (fewer shards, a smaller required set, or caching) goes to Aden **before** any required context changes. | merged #400 |
...
- **Timeline (historical finding):** the original preview-only controls prompted MS-6. #379 connected supported inspector edits to re-rendering; do not treat the old blanket no-export-effect description as current. Each supported control still needs its stated output evidence.
...
| MS-8 | **Smooth camera motion:** FFmpeg `zoompan` on a same-size image steps in whole pixels, which visibly jitters on slow push-ins. Standard fix: upscale the source before `zoompan` (or use a sub-pixel crop/scale expression). | A/B on the same shot: frame-to-frame motion is monotonic without judder (measured motion vectors or pixel shift per frame), render-time cost recorded. | quality | merged #376 for Storyboard; #395 shares the supersampled filter with the job pipeline and adds live A/B judder measurement |
| MS-9 | **Use this PC's GPU to encode:** detect `h264_nvenc` (RTX 2050) and use it for Storyboard and job renders, falling back to `libx264` if the NVENC probe fails. | Same project rendered both ways: time recorded, output passes `media-qa` inspection, fallback proven by forcing a probe failure. | speed | merged #366 |
...
| OpenAI API key | GPT Image models (billed **per token**, not per image: text in $5/1M, image in $8/1M, image out $30/1M — a 1024×1024 medium image is ≈$0.013) | **Sora 2 API deprecated, shuts down 2026-09-24. Do not build.** | [Sora guide](https://developers.openai.com/api/docs/guides/video-generation), [pricing](https://developers.openai.com/api/docs/pricing) |
...
| PROV-3 | **OpenAI API images:** a paid option on the current GPT Image model. Replace or remove any retired DALL·E path in `tools/web.ts` `image_generate`. | A recorded-request test gives the exact endpoint and body; a live frame is made with Aden's key; no retired model ID remains in any list. | #412 open: recorded-request test and token-cost billing landed; the live frame with Aden's key is what remains |
...
**Current implementation:** `workspace/WorkspaceShell.tsx` mounts Explorer, a CodeMirror 6 editor, docked terminal/browser, the Workspace assistant, Changes/proposal review, search and source control. These existing surfaces must survive later IDE integration.
- **Editor:** `CodeEditor.tsx` uses CodeMirror 6, not the former textarea/highlight.js implementation; IDE-1's real-editor acceptance remains the relevant evidence.
- **AI inside the IDE:** `WorkspaceAssistantPanel` is reachable through the Assistant button, using the existing assistant bridge. This does not establish unfinished checkpoint, rules or plan-mode requirements.
- **Source control and search:** `SourceControlPanel` and `SearchPanel` are mounted. Semantic indexing remains the separate IDE-7 requirement; grep is not an embeddings index.
...
| IDE-4 | **Checkpoints:** restore every file an agent run touched to its state before the run. It must be git-aware and never discard user edits made after the run without confirming. | Run → edit by user → restore: the agent's changes revert, the user's later edit is kept or confirmed. | open |
...
Spot-verified against origin/main 6b939e52 and live GitHub state on 2026-09-21 (PR state via gh pr view, content via git grep origin/main with positive controls). Confirmed current: G-2 (#387; nightly green 09-18 through 09-21), G-3 (#382, issue #229 closed), MS-1 #352, MS-2 #375, MS-3 #358, MS-4/MS-9 #366, MS-5 #374, MS-6 #379, MS-7 #359, MS-8 #376/#395, MS-10 #373, MS-13 (all listed PRs merged or closed), PROV-1 #398, PROV-2 #362, IDE-1 #360, IDE-2 #364, IDE-3 #378, IDE-5 #381, IDE-9 #397, IDE-10 #361, REL-3.1 #388, REL-3.2/3.5 #393, REL-3.3 #391, REL-3.4 #403, REL-3.6 #402, REL-4 #363. G-1 updated: two consecutive scheduled runs green, third still required. Undetermined/no change needed this pass: PROV-3/4/5 remain open (DALL-E path still present in web.ts, no Veo shot pipeline found on main), IDE-4/6/7/8/11/12/13 remain open (no corresponding content on main; IDE-11 has open draft #408 with red e2e-all), #296 and #368 remain open, MS-11/MS-12/PROV-6 unchanged (need Aden).
