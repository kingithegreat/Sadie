...
// ── silence console.log from assertPermission / registerTool ─────────────
jest.spyOn(console, 'log').mockImplementation(() => {});

// ── mock assertPermission so tools don't throw in tests ──────────────────
...
describe('tokenize', () => {
  ...
  test('filters tokens shorter than 3 chars', () => {
    ...
    // 'i', 'am', 'a' are <=2 chars → filtered; 'ai' is 2 chars → filtered
...
describe('cosineSim', () => {
  ...
  test('completely disjoint vectors → similarity 0', () => {
...
describe('rag_index handler', () => {
  ...
  test('re-indexing the same file removes stale chunks first', async () => {
    ...
    const r1 = await ragToolHandlers.rag_index({ path: filePath });
    const r2 = await ragToolHandlers.rag_index({ path: filePath });
  ...
  test('fails for missing file', async () => {
    const result = await ragToolHandlers.rag_index({ path: path.join(tmpDir, 'nonexistent.txt') });
    expect(result.success).toBe(false);
    expect(result.error).toMatch(/not found/i);
  });

  test('fails when path points to a directory', async () => {
    const result = await ragToolHandlers.rag_index({ path: tmpDir });
    expect(result.success).toBe(false);
  ...
  test('fails when path is missing', async () => {
    const result = await ragToolHandlers.rag_index({});
    expect(result.success).toBe(false);
...
describe('rag_query handler', () => {
  async function indexDoc(name: string, content: string) {
    ...
    const r = await ragToolHandlers.rag_index({ path: p });
  ...
  test('returns empty-index error when nothing indexed', async () => {
    ...
    // Actually each test has fresh tmpDir & handlers share module state — so let's just verify error message
    const r = await ragToolHandlers.rag_query({ query: 'test query' });
    if (!r.success) {
      expect(r.error).toMatch(/no documents indexed/i);
    } else {
      // Some data pre-exist from prior test in same suite — that's fine
  ...
  test('returns top-k results after indexing', async () => {
    ...
    const result = await ragToolHandlers.rag_query({ query: 'capital France Paris', top_k: 3 });
    expect(result.success).toBe(true);
  ...
  test('most relevant chunk ranks first', async () => {
    ...
    await ragToolHandlers.rag_index({ path: filePath });

    const r = await ragToolHandlers.rag_query({ query: 'quantum computing qubits superposition', top_k: 3 });
  ...
  test('top_k caps at 10', async () => {
    ...
    await ragToolHandlers.rag_index({ path: p });
    const r = await ragToolHandlers.rag_query({ query: 'word sentence', top_k: 50 });
  ...
  test('doc_id filter restricts to one document', async () => {
    ...
    const r = await ragToolHandlers.rag_query({ query: 'TypeScript React', doc_id: docId, top_k: 5 });
    expect(r.success).toBe(true);
  ...
  test('doc_id filter for unknown id returns error', async () => {
    ...
    const r = await ragToolHandlers.rag_query({ query: 'anything', doc_id: 'rag:nonexistent' });
    expect(r.success).toBe(false);
    expect(r.error).toMatch(/no chunks found/i);
  });

  test('missing query returns error', async () => {
    const r = await ragToolHandlers.rag_query({ top_k: 3 });
    expect(r.success).toBe(false);
...
describe('rag_list handler', () => {
  test('returns empty list message when nothing indexed', async () => {
    // Module-level ragStore may have data — test both branches
    const r = await ragToolHandlers.rag_list({});
    expect(r.success).toBe(true);
