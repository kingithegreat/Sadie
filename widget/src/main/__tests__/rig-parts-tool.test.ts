...
describe('media_generate_rig_parts', () => {
  ...
  it('sends the guide first with the character art, snaps the part, and writes PartDef values', async () => {
    ...
    const result = await generateRigParts({ character: 'leila', description: 'Black puffed sleeve.', parts: ['upper_arm_l'] }, deps(async (prompt, images, aspect) => {
  ...
  it('retries a rejected attempt with the reason, and reports a part that never passes as failed', async () => {
    ...
    const result = await generateRigParts({ character: 'leila', description: 'd', parts: ['forearm_l'], maxAttempts: 2 }, deps(async prompt => {
      prompts.push(prompt);
  ...
  it('a failed request is not retried and costs nothing', async () => {
    ...
    const result = await generateRigParts({ character: 'leila', description: 'd', parts: ['upper_arm_r'] }, deps(generate));
  ...
  it('refuses reference images outside the characters folder, and unknown parts, before any request', async () => {
    ...
    const part = await generateRigParts({ character: 'leila', description: 'd', parts: ['tail'] }, deps(generate));
