...
export function registerStudioIpc(
  ...
  ipcMain.handle('homebot:media:storyboard:render', async (_ev, args: { projectId: string; sceneId?: string; motion?: boolean; burnSubtitles?: boolean; outputSpec?: unknown; variantId?: unknown; narrationEngine?: unknown; colorGrade?: string; musicEnabled?: boolean; musicTrack?: string; musicVolume?: number; encoder?: 'auto' | 'nvenc' | 'cpu' }) => {
    try {
      const res = await invokeTool(_ev, 'media_render_storyboard', {
        ...
        ...(args.musicTrack === undefined ? {} : { musicTrack: args.musicTrack }),
        ...(args.musicVolume === undefined ? {} : { musicVolume: args.musicVolume }),
        ...(args.encoder === undefined ? {} : { encoder: args.encoder }),
      });
