...
// E2E detection: enable only when HOMEBOT_E2E is explicitly set.
...
export const isE2E = process.env.HOMEBOT_E2E === '1' || process.env.HOMEBOT_E2E === 'true';

export const isDemoMode = process.argv?.includes('--demo') || process.env.HOMEBOT_DEMO_MODE === '1' || process.env.HOMEBOT_DEMO_MODE === 'true';

// Runtime mode: determines security and feature levels
export type RuntimeMode = 'demo' | 'beta' | 'prod';
export function getRuntimeMode(): RuntimeMode {
  if (isDemoMode) return 'demo';
  if (process.argv?.includes('--beta') || process.env.HOMEBOT_BETA === '1') return 'beta';
  return 'prod';
}
...
// preserve E2E mode when `HOMEBOT_E2E` or `NODE_ENV=test` is set so that
...
export function sanitizeEnvForPackaged() {
  ...
  if (process.env.HOMEBOT_E2E) {
    console.warn('[HomeBot] Removing HOMEBOT_E2E in release build');
    delete process.env.HOMEBOT_E2E;
  }
  if (process.env.HOMEBOT_DIRECT_OLLAMA) {
    console.warn('[HomeBot] Removing HOMEBOT_DIRECT_OLLAMA in release build');
    delete process.env.HOMEBOT_DIRECT_OLLAMA;
  }
  // Do not allow demo mode to be set via ENV in release builds
  if (process.env.HOMEBOT_DEMO_MODE) {
    console.warn('[HomeBot] Removing HOMEBOT_DEMO_MODE in release build');
