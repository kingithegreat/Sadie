...
import { assembleShotsForScene } from './storyboard-assembly';
...
export interface StoryboardRenderOptions {
  ...
  outputName?: string;
  /** Main-process snapshot of the saved scene shown in Studio. */
  savedShots?: ShotManifest[];
  /** Optional aspect ratio (defaults to 16:9 widescreen) */
...
export function buildKenBurnsFilter(movement: string, durationSec: number, fps = 30, w = 1920, h = 1080): string {
  const frames = Math.max(1, Math.round(durationSec * fps));
  const move = movement.toLowerCase().trim();

  if (move === 'slow push in') {
    // Zoom from 1.0 to 1.25 toward center
    return `zoompan=z='min(zoom+0.0015,1.25)':d=${frames}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=${w}x${h}:fps=${fps}`;
  } else if (move === 'pan right') {
    // Constant 1.15 scale with horizontal rightward pan
    return `zoompan=z='1.15':x='if(lte(on,1),(iw-iw/zoom)/2,min(x+1.5,iw-iw/zoom))':y='ih/2-(ih/zoom/2)':d=${frames}:s=${w}x${h}:fps=${fps}`;
  } else if (move === 'tilt up') {
    // Constant 1.15 scale with upward vertical tilt
    return `zoompan=z='1.15':x='iw/2-(iw/zoom/2)':y='if(lte(on,1),(ih-ih/zoom)/2,max(y-1.5,0))':d=${frames}:s=${w}x${h}:fps=${fps}`;
  } else if (move === 'tracking') {
    // Slight zoom with diagonal flow
    return `zoompan=z='min(zoom+0.001,1.18)':x='if(lte(on,1),(iw-iw/zoom)/2,min(x+1.2,iw-iw/zoom))':y='ih/2-(ih/zoom/2)':d=${frames}:s=${w}x${h}:fps=${fps}`;
  } else {
    // Static locked shot: scale to fill cleanly
...
export async function renderStoryboardMovie(
  ...
  if (opts.savedShots !== undefined) {
    shots = opts.savedShots;
  } else {
    const manifestPath = path.join(projectDir, 'scenes', sceneId, 'manifest.json');
    if (fs.existsSync(manifestPath)) {
      try {
        shots = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'));
      } catch (e: any) {
        return { ok: false, error: `Could not parse scene manifest: ${e.message}` };
      }
    } else {
      shots = assembleShotsForScene(projectDir, sceneId) as unknown as ShotManifest[];
    }
  }
  ...
  const targetW = opts.aspectRatio === '9:16' ? 1080 : opts.aspectRatio === '1:1' ? 1080 : 1920;
  const targetH = opts.aspectRatio === '9:16' ? 1920 : opts.aspectRatio === '1:1' ? 1080 : 1080;
  ...
  try {
    ...
    if (motion) {
      ...
      for (let i = 0; i < shots.length; i++) {
        ...
        const kbFilter = buildKenBurnsFilter(shot.movement || 'static', dur, 30, targetW, targetH);
    ...
    if (!facts.hasVideo || facts.width !== targetW || facts.height !== targetH || !facts.hasAudio ||
