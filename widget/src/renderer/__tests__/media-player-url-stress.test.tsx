...
describe('toMediaFileUrl — hostile path inputs', () => {
  test.each([
    ...
    const out = toMediaFileUrl(input as string);
    if (expected instanceof RegExp || typeof expected === 'object' && expected !== null && !Array.isArray(expected)) {
  ...
  test('round-trips back to a path a browser would resolve identically', () => {
    // The whole point: decoding the URL must give the original path.
...
describe('Media Studio timeline media wiring', () => {
  ...
  test('a rendered job with a hostile Windows path loads an escaped, playable URL', async () => {
