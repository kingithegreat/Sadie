...
export default function CapabilityReport({ onNavigate }: CapabilityReportProps = {}) {
  ...
  const handleCopy = useCallback(async (id: string, command: string) => {
    // Route through main. This window's permission handler allows only
    // media/microphone/audioCapture, so `navigator.clipboard.writeText` was
    // denied — and because the old code set `copiedId` *after* the try block
    // as well as inside it, the button said "Copied" either way. That is the
    // defect: feedback that cannot distinguish success from failure. It now
    // follows the real result and says so when the write failed.
