  ...
  const handleSaveMouthAnchor = async () => {
    ...
    try {
      const res = await api()?.mediaAncientPathwaysSaveAnchor?.({
        ...
        pose: selectedPose,
        anchorType: 'mouth',
        box: [mouthAnchor.x, mouthAnchor.y, mouthAnchor.w, mouthAnchor.h],
      });
      if (res?.ok) {
        setIsMouthSuggested(false);
        showToast(`Saved mouth anchor for ${selectedCharSlug} [${selectedPose}]`);
        await loadCharacters(selectedCharSlug);
      } else {
        showToast(res?.error || 'Failed to save mouth anchor', 'error');
      }
    } catch (err: any) {
  ...
  const handleSaveHeadBox = async () => {
    ...
    try {
      const res = await api()?.mediaAncientPathwaysSaveAnchor?.({
        ...
        pose: selectedPose,
        anchorType: 'head',
        box: [headBox.x, headBox.y, headBox.w, headBox.h],
      });
      if (res?.ok) {
        showToast(`Saved head box for ${selectedCharSlug} [${selectedPose}]`);
        await loadCharacters(selectedCharSlug);
      } else {
        showToast(res?.error || 'Failed to save head box', 'error');
      }
    } catch (err: any) {
      showToast(err?.message || 'Save error', 'error');
    } finally {
