...
test('Studio exports a real local movie with timed narration and captions', async ({}, testInfo) => {
  ...
  const scene = path.join(projectDir, 'scenes', 'scene_01');
  ...
  const shots = lines.map((narration, index) => {
    const shotId = `shot_0${index + 1}`;
    ...
    const shot = { shotId, order: index + 1, prompt: narration, framing: 'wide', lens: '35mm', movement: 'static', durationSec: 4, narration, status: 'IMAGE_GENERATED', frameImagePath };
  ...
  fs.writeFileSync(path.join(scene, 'scene.json'), JSON.stringify({ sceneId: 'scene_01', shots: shots.map(s => s.shotId) }));
  fs.writeFileSync(path.join(scene, 'manifest.json'), JSON.stringify(shots));
  ...
  try {
    ...
    const trapSpeechNetwork = async () => expect(await app.evaluate(() => {
